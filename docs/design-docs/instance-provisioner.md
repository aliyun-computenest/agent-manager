...
|--------|-----------|---------|
| `provisionInstance(input)` | `services/instance-provisioner.js` | Main entry, returns `{ instance, sandboxUrl, hosts? }` |
...
sequenceDiagram
  ...
  IP-->>R: {instance, sandboxUrl, hosts}
```
...
| `running` | provisioner post-sandbox | Sandbox up, gateway authorized |
