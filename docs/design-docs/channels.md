...
sequenceDiagram
  ...
  S->>IM: register webhook(agent gateway URL)
...
- Credentials are encrypted **before** the row hits Supabase — the only
  decrypt site is the agent gateway when issuing webhook calls.
...
| `AGENT_GATEWAY_URL` | each `*-auto-config.js` | Webhook URL registered with the IM platform |
...
- [`agent-manager/deploy/agent-gateway/`](../../agent-manager/deploy/agent-gateway/) — receives webhook traffic
