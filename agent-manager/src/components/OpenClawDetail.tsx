...
import { apiUrl, nativeAgentUiEnabled } from '../lib/api'
...
// 历史上实例详情页只服务 OpenClaw，因此组件沿用了 OpenClawDetail 命名。
// 目前管理员和普通用户的各类 Agent 实例详情都复用本组件，具体能力由 agent_type 决定。
...
const OpenClawDetail: React.FC = () => {
  ...
  // 原生界面依赖正在运行的 Sandbox；这里只决定是否展示入口，
  // 用户权限和实例状态会在 preview-session 接口中再次校验。
  const canOpenNativeUi = nativeAgentUiEnabled
    && instance.status === 'running'
    && Boolean(instance.sandbox_id)
  ...
  const handleOpenNativeUi = () => {
    if (!canOpenNativeUi) return
    navigate(`${isAdminView ? '/admin' : '/user'}/instances/${instance.id}/native-ui`)
  }
  ...
  return (
    <div className="min-w-0 w-full max-w-full space-y-6 overflow-x-hidden">
      ...
      <div className="flex min-w-0 flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center gap-3 xl:w-auto xl:justify-end">
          <button
            type="button"
        ...
        <div className="grid min-w-0 grid-cols-1 gap-6 md:grid-cols-2">
          ...
          {canOpenNativeUi && (
            <div
              ...
              data-testid="instance-app-access"
            >
              {isAdminView && (
                <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <div className="flex min-w-0 flex-wrap items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={handleOpenNativeUi}
                  data-testid="native-agent-ui-open"
                  className="btn-secondary flex items-center gap-2"
                >
                  <ExternalLink className="h-5 w-5" />
                  <span>{t('openClawDetail.nativeUi.open')}</span>
                </button>
              </div>
            </div>
          )}
