...
export default function FeishuQRConfig({ instanceId, onSuccess, onCancel }: FeishuQRConfigProps) {
  ...
  // Start the Feishu device registration flow
  const startFlow = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${apiUrl}/api/channel-auto-config/feishu/begin`, {
        method: 'POST',
        headers: {
      ...
      if (!data.success) {
        throw new Error(data.error || t('qrConfig.feishu.startFailed'))
      }
      ...
      setStatus('error')
      setErrorMsg(err.message || t('qrConfig.feishu.startFailed'))
    }
  ...
  const pollStatus = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${apiUrl}/api/channel-auto-config/feishu/poll`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${session.access_token}`
  ...
  return (
    <div className="border border-blue-200 rounded-lg p-6 bg-blue-50/50">
      <div className="flex items-center space-x-2 mb-4">
        ...
        <h4 className="font-medium text-gray-900">{t('qrConfig.feishu.title')}</h4>
      </div>
      ...
      {status === 'scanning' && verificationUrl && (
        <div className="flex flex-col items-center">
          ...
          <p className="text-sm text-gray-700 mb-1">
            ...
            <span className="font-semibold text-blue-600">{t('qrConfig.feishu.appName')}</span>
      ...
      {status === 'success' && (
        <div className="flex flex-col items-center py-6">
          ...
          <p className="text-xs text-gray-500 mt-1">{t('qrConfig.feishu.successHint')}</p>
        </div>
      )}
