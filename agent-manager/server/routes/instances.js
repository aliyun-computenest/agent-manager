...
import {
  ...
  NATIVE_AGENT_UI_PROXY_BASE_PATH,
  NATIVE_AGENT_UI_PREVIEW_COOKIE,
  NATIVE_AGENT_UI_PREVIEW_TARGET_COOKIE
...
const InstanceWithExtrasSchema = InstanceSchema.extend({
  ...
  e2bHost: z.string().nullable().optional().describe('管理员排障用的 E2B Host，不包含运行时 token'),
  principal: z.object({
...
const PreviewSessionBodySchema = z.object({
  previewKey: z.string().regex(/^[A-Za-z0-9_-]{24}$/).optional()
}).optional()
...
defineRoute(router, {
  ...
  const agentPort = await resolveAgentPort(instance.agent_type_id, 'instance detail port lookup')
  const e2bHost = instance.sandbox_id
    ? buildE2BUpstreamHost({
      agentPort,
      sandboxId: instance.sandbox_id
    })
  ...
  const shouldShowHosts = DEPLOY_ENVIRONMENT === 'local-dev' || E2B_NEEDS_HOSTS
  if (instance.sandbox_id && shouldShowHosts && E2B_HOSTS_IP) {
    hostsEntries = e2bHost ? [`${E2B_HOSTS_IP} ${e2bHost}`] : null
  }
  ...
  res.json({
    ...
    instance: {
      ...
      // E2B Host 只用于管理员排障，不向普通用户响应。
      ...(isPlatformAdminProfile(req.userProfile) ? { e2bHost } : {}),
      sandboxStatus,
...
/**
 * 为原生 UI iframe 和 WebSocket 建立短期 OAuth 预览会话。
 * iframe 与本接口同属 Manager Origin，浏览器会自动向 /_preview/ 携带 HttpOnly Cookie。
 */
defineRoute(router, {
  method: 'post',
  path: '/instances/{instanceId}/preview-session',
  operationId: 'createInstancePreviewSession',
  tags: ['Instances'],
  summary: '建立 Agent 原生界面预览会话',
  security: [{ bearerAuth: [] }],
  request: {
    params: InstanceIdParamsSchema,
    body: { content: { 'application/json': { schema: PreviewSessionBodySchema } } }
  },
  responses: {
    200: {
      description: '预览会话已建立',
      content: { 'application/json': { schema: PreviewSessionResponseSchema } }
    },
    401: errorResponse,
    404: errorResponse,
    409: errorResponse,
  ...
  if (!NATIVE_AGENT_UI_ENABLED || instance.status !== 'running' || !instance.sandbox_id) {
    return res.status(409).json({ success: false, error: 'Native Agent preview is not available' })
  }

  const agentPort = await resolveAgentPort(instance.agent_type_id, 'native preview session port lookup')
  // OAuth token 刷新时复用当前 key，只续期该标签页路径下的 Cookie，不重载 Agent。
  const previewKey = req.body?.previewKey || randomBytes(18).toString('base64url')
  const previewUrl = buildNativeAgentUiProxyUrl({
    managerOrigin: PLATFORM_PUBLIC_URL,
  ...
  if (!previewUrl) {
    return res.status(409).json({ success: false, error: 'Native Agent preview is not configured' })
  }
  const previewTarget = buildNativeAgentUiPreviewTarget({
    previewKey,
    agentPort,
    sandboxId: instance.sandbox_id
  })
  ...
  const cookieOptions = {
    ...
    sameSite: 'lax',
    secure,
    // 每个 preview key 使用独立 Cookie Path，同一浏览器可同时打开多个 Agent。
    path: `${NATIVE_AGENT_UI_PROXY_BASE_PATH}/${previewKey}/`
  }
  try {
    const payload = JSON.parse(Buffer.from(req.userToken.split('.')[1], 'base64url').toString('utf8'))
    const remainingSeconds = Math.floor(Number(payload.exp) - Date.now() / 1000)
    if (Number.isFinite(remainingSeconds) && remainingSeconds > 0) {
      cookieOptions.maxAge = Math.min(remainingSeconds, 3600) * 1000
    }
  } catch {
    // requireAuth 已验证 token；无法读取 exp 时退化为浏览器会话 Cookie，
    // 后续每次代理请求仍会重新校验，不扩大授权时间。
  }

  res.setHeader('Cache-Control', 'no-store')
  // 旧 key 的 Path Cookie 最长一小时后自然过期；不能用 path=/ 清除，
  // 否则只会生成一个无关的过期 Cookie，并不会删除标签页独立 Cookie。
  res.cookie(NATIVE_AGENT_UI_PREVIEW_COOKIE, req.userToken, cookieOptions)
  res.cookie(NATIVE_AGENT_UI_PREVIEW_TARGET_COOKIE, previewTarget, cookieOptions)
...
defineRoute(router, {
  ...
  try {
    ...
    if (sandbox) {
      const startAgentPort = await resolveAgentPort(instance.agent_type_id, 'start host verification port lookup')
      try {
