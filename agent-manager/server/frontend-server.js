...
const server = http.createServer((req, res) => {
  // 8080 只负责把保留路径交给后端；功能开关和配置完整性由后端统一判断。
  if (parseNativeAgentUiProxyPath(req.url) !== null) {
    proxyHttp(req, res)
    return
  }
...
server.on('upgrade', (req, socket, head) => {
  const isNativeAgentPreview = parseNativeAgentUiProxyPath(req.url) !== null
  if (!isNativeAgentPreview && !(req.url || '').startsWith('/api')) {
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`Native Agent runtime proxy path forwarded at ${NATIVE_AGENT_UI_PROXY_BASE_PATH}/`)
})
